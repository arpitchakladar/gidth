...
pub fn siphon_traits(_attr: TokenStream, trait_item: TokenStream) -> TokenStream {
	...
	let supertraits = parsed_trait.supertraits.clone(); // Avoid borrowing issues

	// Store extracted traits and methods in the registry
	if let Ok(mut registry) = DERIVED_TRAIT_REGISTRY.lock() {
		registry.insert(
			trait_name.clone(),
			(base_traits, method_signatures, quote! { #supertraits }.to_string()),
		);
	}
