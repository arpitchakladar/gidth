...
fn u_exp_small_in(base: &BigDecimal, power: usize, result: BigDecimal, current: BigDecimal) -> (BigDecimal, BigDecimal) {
	if power == 0 {
...
impl BigDecimal {
	pub fn exp(&self, power: usize) -> BigDecimal {
		...
		buf1.limbs.push(1);
		buf2.limbs.push(1);
		let mut result = u_exp_small_in(self, power, buf1, buf2).0;
